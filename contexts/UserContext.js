...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadUserFromStorage = async () => {
      try {
        ...
        const userSaldo = await AsyncStorage.getItem('userSaldo');
        ...
        if (userId && userMatricula) {
          ...
          if (!error && userData && userData.length > 0) {
            ...
            setUser(u);
            setToken(`token-${u.id}`);
            setSaldo(parseFloat(userSaldo) || parseFloat(u.saldo) || 0.0);
          } else {
            await logout();
          }
        }
  ...
  const signIn = async (identifier, password) => {
    ...
    try {
      ...
      await AsyncStorage.setItem('userSaldo', String(parseFloat(u.saldo) || 0.0));
      setToken(`token-${u.id}`);
